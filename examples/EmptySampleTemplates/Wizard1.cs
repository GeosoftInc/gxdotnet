...
// CREATED FROM: Geosoft Wizard GX Template version 7.0.0
...
namespace EmptySampleTemplates
{
   ...
   //
   //
   //       3. Add GX Controls and any other needed Windows forms controls from the toolbox to the wizard's pages.
   //          The wizard will be automatically resizzed during runtime. Give each control a funtional name indicating
   //          that it is a member of the form and what kind of control it is (e.g. m_cmbAssayList for a combobox
   //          containing a list of assay options). This makes for readable and maintainable code.
   //
   ...
   public partial class Wizard1 : BaseWizard
   {
      ...
      : base()
      {
         ...
         // TODO: Add any pages in order to the Pages property here and in any other constructors
         this.Pages.Add(m_wizardPage1);
         this.Pages.Add(m_wizardPage2);
         this.Pages.Add(m_wizardPage3);
         this.Pages.Add(m_wizardPage4);
      }
      ...
      public override bool bValidateDialog(int iPage, ref string strINIError, bool bUseErrorProvider)
      {
         bool bValid = true;
         ...
         if (!base.bValidateDialog(iPage, ref strINIError, bUseErrorProvider))
            bValid = false;

         // Never use error provider with full validation
         if (iPage == -1)
            bUseErrorProvider = false;
         ...
         if (bValid)
         {
            ...
            //       2. The iPage parameter indicates that the wizard is about to leave this page and that only controls on
            //          this page should be validated.
            //       3. If iPage is -1 the all controls should be validated (scripting etc.)
            //       4. Create a string describing the problem and if the bUseErrorProvider parameter is true set this
            //          error and the control in question to the base class' ControlErrorProvider.SetError method.
            //       5. Append an error string to the strINIError string parameter decribing the problem. The convenient
      ...
      protected override void UpdateState()
      {
         ...
         // TODO: Add code here if you want to change the state of any controls based
         //       on user choices in the previous page, for example:
      ...
      private void Execute()
      {
         ...
         string strError = String.Empty;
         if (!bValidateDialog(-1, ref strError, false))
      ...
      //
      //       Note: The default behaviour for the Next/Back/Cancel buttons is handled by the BaseWizard internals.
      //       Should any extra logic be needed it can be added to the BaseWizard overrides above.
   }
}